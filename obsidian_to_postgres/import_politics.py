...
class Politics:
    def __init__(self, db_config_path):
        """
        Initialize the Politics class with database connection parameters.
...
# Example usage (if used in a script or main):
if __name__ == "__main__":
    directory_path = "C:/Users/larch/iCloudDrive/iCloud~md~obsidian/WizeCosm/00 - Univers/Politique/"
    db_config_path = "postgres_params.json"
    table_name = "Politics"

    Politics = Politics(db_config_path)
    df = Politics.process_markdown_files_in_directory(directory_path)
    print(df)
    Politics.save_to_postgresql(df, table_name)
